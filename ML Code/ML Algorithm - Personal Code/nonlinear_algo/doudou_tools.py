...
def get_variance(list_v):
    mean = get_mean(list_v)
    res = 0
    for item in list_v:
        res += math.pow((item - mean),2)
    return (res/len(list_v))

#population std -> but use numpy now we would rather use
# the sample std not population std
def get_stdev(list_v):
    lol = math.sqrt(get_variance(list_v))
    print lol
    return lol
